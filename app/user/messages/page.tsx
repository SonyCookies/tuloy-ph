...
import RatingModal from "@/app/components/ui/RatingModal";
...
function StudentMessagesContent() {
  ...
  const [ratingModalData, setRatingModalData] = useState({ dormId: '', dormName: '', msgId: '' });
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch Inquiries (Threads)
        const { data: inquiriesData, error: inquiriesError } = await supabase
          .from('Inquiries')
          .select('*, Messages(sender_id, is_read, content, type, created_at, metadata), VisitSchedule(sender_id, receiver_id, is_read, status), Dorm(images)')
          .eq('user_id', user.id)
          .order('created_at', { ascending: false });

        if (inquiriesError) throw inquiriesError;
        
        const fetched = (inquiriesData || []).map((inq: any) => {
          ...
          // Calculate unread count for this inquiry
          const unreadMsgs = (inq.Messages || []).filter((m: any) => !m.is_read && m.sender_id !== user.id).length;
          const unreadVs = (inq.VisitSchedule || []).filter((vs: any) => {
             if (vs.is_read) return false;
             // If I am the receiver and it's pending/any, it's unread
             if (vs.receiver_id === user.id) return true;
             // If I am the sender and it's NOT pending (meaning it was accepted/declined), it's unread
          ...
          return {
            ...
            unread_count: unreadMsgs + unreadVs
          };
        });

        setInquiries(fetched);

        // Batch-fetch owner profiles for all unique owner_ids
        const ownerIds = [...new Set(fetched.map((i: any) => i.owner_id).filter(Boolean))];
        if (ownerIds.length > 0) {
          ...
          if (owners) {
            ...
            owners.forEach((o: any) => { profileMap[o.id] = o; });
    ...
    const msgChannel = supabase.channel('sidebar-messages')
      .on('postgres_changes', {
        event: '*', // INSERT or UPDATE (for read status)
        schema: 'public',
        table: 'Messages'
      }, () => fetchData())
      .subscribe();

    const vsChannel = supabase.channel('sidebar-visits')
      .on('postgres_changes', {
        event: '*', 
        schema: 'public',
        table: 'VisitSchedule'
  ...
  // Fetch owner profile + messages when a thread is selected
  useEffect(() => {
    ...
    const fetchOwner = async () => {
      if (!selectedInquiry.owner_id) return;
      const { data } = await supabase
        .from('User')
        .select('name, image')
        .eq('id', selectedInquiry.owner_id)
        .single();
    ...
    // Runs every 5 seconds, but ONLY fetches messages created AFTER our last known message
    const intervalId = setInterval(async () => {
      ...
      if (data && data.length > 0) {
        // Only trigger re-render if we actually received NEW messages
        setMessages(prev => {
    ...
    const channel = supabase
      .channel(`user-chat-${selectedInquiry.id}`)
      .on('postgres_changes', { 
        event: '*', 
        schema: 'public', 
        table: 'Messages',
        ...
        if (payload.eventType === 'INSERT') {
          if (payload.new.sender_id !== userData?.id) {
            markAsRead(selectedInquiry.id);
          }
          setMessages(prev => {
    ...
    const vsChannel = supabase
      .channel(`user-vs-${selectedInquiry.id}`)
      .on('postgres_changes', { 
        event: '*', 
        schema: 'public', 
        table: 'VisitSchedule',
  ...
  const handleScheduleProposal = async (date: Date, time: string) => {
    if (!selectedInquiry || !userData) return;
    ...
    try {
      ...
      if (confirmed && confirmed.length > 0) {
        setSuccessModalData({
          title: 'Visit Already Confirmed',
          message: 'You already have an active visit confirmed for this property. If you need to reschedule, please manage it through your calendar.'
        });
      ...
      const { error } = await supabase
        ...
        .insert([{ 
          ...
          receiver_id: selectedInquiry.owner_id,
  ...
  const handleAcceptProposal = async (scheduleId: string) => {
    try {
      ...
      if (acceptedMsg?.metadata) {
        // Keeping this for backward compatibility until calendar is migrated, optional
        await supabase
          .from('Inquiries')
          .update({
      ...
      setInquiries(prev => {
        ...
        return [...updated].sort((a, b) => 
          new Date(b.latest_message_time || b.created_at).getTime() - new Date(a.latest_message_time || a.created_at).getTime()
        );
      });

      setSuccessModalData({
        title: 'Visit Accepted!',
        message: `You've successfully confirmed a visit for ${acceptedMsg?.metadata?.date ? format(new Date(acceptedMsg.metadata.date), 'MMMM d') : 'the requested date'} at ${acceptedMsg?.metadata?.time}.`
      });
      setIsSuccessModalOpen(true);
  ...
  // Hide BottomNav when in chat view
  useEffect(() => {
    if (view === 'chat') {
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!newMessage.trim() || !selectedInquiry || !userData) return;
    ...
    try {
      const { data, error } = await supabase
        .from('Messages')
        .insert([{ inquiry_id: selectedInquiry.id, sender_id: userData.id, content }])
        .select();
  ...
  const handleSelectThread = (inquiry: any) => {
    router.push(`/user/messages?id=${inquiry.id}`);
  ...
  return (
    <>
      <div className="max-w-7xl mx-auto h-screen flex flex-col md:flex-row relative">
          ...
          <div className="p-6 pb-8 bg-transparent">
            <div className="flex justify-between items-end mb-8">
               <div>
                  ...
                  <p className="text-gray-500 font-bold text-[10px] uppercase tracking-[0.15em] opacity-60">Your Inquiries</p>
               </div>
             </div>
        ...
        {/* Conversation List */}
        <div className="flex-1 overflow-y-auto px-4 pb-32 md:pb-6 scrollbar-hide space-y-3">
          {loading ? (
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div className="relative flex-shrink-0">
                        <div className={`w-14 h-14 rounded-full overflow-hidden border-2 p-0.5 transition-colors ${
                          ...
                          {ownerProfiles[inquiry.owner_id]?.image ? (
                            <div className="relative w-full h-full rounded-full overflow-hidden">
                              <Image
                                src={ownerProfiles[inquiry.owner_id].image}
                                alt="Profile"
                                fill
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between mb-1">
                          <h4 className={`text-sm truncate ${
                            ...
                            {ownerProfiles[inquiry.owner_id]?.name || 'Loading...'}
                          </h4>
                        ...
                        <div className="flex items-center justify-between gap-3">
                          ...
                          {hasUnread && inquiry.unread_count > 0 && (
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="relative">
                    <div className="w-10 h-10 rounded-full bg-white/5 overflow-hidden relative border border-white/10">
                      {ownerProfile?.image ? (
                        <Image src={ownerProfile.image} alt="" fill className="object-cover" />
                      ) : (
                        <div className="w-full h-full flex items-center justify-center font-black text-gray-400 text-xs">
                          {ownerProfile?.name?.[0] || selectedInquiry.dorm_name?.[0]}
                        </div>
                      )}
                    </div>
                  ...
                  <div>
                    ...
                    <p className="text-[10px] font-bold text-primary uppercase tracking-widest opacity-80">
                      Owner of {selectedInquiry.dorm_name}
                    </p>
                  </div>
                </div>
              </header>
                ...
                >
                  <div className="max-w-[90%] sm:max-w-[80%] flex flex-col items-start">
                    <div className="bg-white/5 text-neutral-dark rounded-[1.5rem] rounded-tl-[0.4rem] overflow-hidden border border-white/10 shadow-sm">
                      ...
                      <Link 
                        href={`/user/dorms/${selectedInquiry.dorm_id}`}
...
{/* Real-time Messages Loop */}
                <AnimatePresence>
                  {(() => {
                    ...
                    return combinedItems.map((msg, idx) => {
                        ...
                        return (
                          ...
                          >
                            <div className="flex flex-col items-center gap-2">
                              ...
                              >
                                View My Calendar <ChevronRight className="w-2.5 h-2.5" />
                              </Link>
                            </div>
                          </motion.div>
                        );
                      ...
                      if (msg.type === 'system_rating_request') {
                        const isCompleted = msg.metadata?.status === 'completed';
                        return (
                          <motion.div 
                            initial={{ opacity: 0, y: 10 }}
                            animate={{ opacity: 1, y: 0 }}
                            key={msg.id}
                            className="flex flex-col items-center w-full my-8 bg-transparent"
                          >
                            <div className="bg-white rounded-[2rem] p-6 shadow-xl shadow-black/[0.03] border border-gray-50 flex flex-col items-center text-center max-w-[280px]">
                              <div className="w-12 h-12 bg-amber-50 rounded-2xl flex items-center justify-center text-amber-500 mb-4 shadow-inner">
                                <Star className={`w-6 h-6 ${isCompleted ? 'fill-current' : ''}`} />
                              </div>
                              <h3 className="text-sm font-black text-neutral-dark mb-1">How was the visit?</h3>
                              <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest leading-relaxed mb-6 px-2">
                                {isCompleted 
                                  ? "Thanks for your feedback!\nYou've rated this property." 
                                  : "Your feedback helps others.\nRate the unit now or later."}
                              </p>
                             ...
                             {!isCompleted && (
                                <button 
                                  onClick={() => {
                                    setRatingModalData({ 
                                      dormId: msg.metadata?.dorm_id, 
                                      dormName: msg.metadata?.dorm_name || selectedInquiry.dorm_name,
                                      msgId: msg.id
                                    });
                                    setIsRatingModalOpen(true);
                                  }}
                                  className="w-full bg-amber-400 hover:bg-neutral-dark text-white py-3 rounded-2xl font-black uppercase tracking-[0.2em] text-[9px] shadow-lg shadow-amber-400/20 transition-all flex items-center justify-center gap-2"
                                >
                                  Rate Now & Save <ChevronRight className="w-3.5 h-3.5" />
                                </button>
                             )}
                            </div>
                          </motion.div>
                                ...
                                <div className="space-y-3 min-w-[200px] py-1">
                                   ...
                                   <div className="flex flex-col gap-2">
                                      {msg.metadata?.status === 'pending' && !isMine ? (
                                        <>
                                          <button onClick={() => handleAcceptProposal(msg.id)} className="w-full bg-secondary text-white py-2 rounded-xl text-[10px] font-black uppercase tracking-widest shadow-lg shadow-black/5 hover:scale-105 active:scale-95 transition-all">Accept</button>
              ...
              <footer className="p-4 bg-white backdrop-blur-md border-t border-gray-100 sticky bottom-0 w-full z-20 mt-auto flex-shrink-0">
                <div className="max-w-4xl mx-auto mb-3 flex justify-start">
                   {(() => {
                     const hasActiveVisit = visitSchedules.some(vs => vs.status === 'pending' || vs.status === 'accepted');
                     return (
                       <button 
                       ...
                       >
                         <Calendar className="w-3.5 h-3.5" />
                         {hasActiveVisit ? 'Visit Scheduled' : 'Schedule Visit'}
                       </button>
                     );
                   })()}
                </div>
              ...
              >
                {/* Input Box: Soft gray background for contrast against the white footer */}
                <div className="flex-1 bg-gray-50/80 border border-gray-200/50 rounded-[1.5rem] px-4 py-1.5 flex items-end focus-within:bg-white focus-within:ring-2 focus-within:ring-primary/5 focus-within:border-primary/10 transition-all shadow-sm">
                  <textarea 
                    rows={1}
                    value={newMessage}
                ...
                {/* Submit Button: Always visible, changes color based on input state */}
                <button 
                  type="submit"
            ...
            <div className="flex-1 flex flex-col items-center justify-center p-12 bg-transparent">
               ...
               <div className="text-center space-y-4 max-w-sm">
                  ...
                  <p className="text-xs font-bold text-gray-400 leading-relaxed uppercase tracking-widest opacity-60">
                    Select a conversation from the sidebar to view details and message the property owner.
                  </p>
      ...
      <RatingModal 
        ...
        dormId={ratingModalData.dormId}
        dormName={ratingModalData.dormName}
        onRated={async () => {
          setMessages(prev => prev.map(m => 
            m.id === ratingModalData.msgId ? { ...m, metadata: { ...m.metadata, status: 'completed' } } : m
          ));
          
          await supabase
            .from('Messages')
            .update({ metadata: { ...ratingModalData, status: 'completed' } })
            .eq('id', ratingModalData.msgId);
        }}
      />
      ...
      <Modals 
        ...
        handleScheduleProposal={handleScheduleProposal}
      />
    ...
    <>
      {selectedInquiry && (
        <ScheduleVisitModal 
          ...
          role="user"
          onConfirm={handleScheduleProposal}
        />
      )}
    </>
  );
