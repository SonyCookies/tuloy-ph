...
function CalendarContent() {
    ...
    const fetchVisits = async () => {
      try {
        ...
        const { data, error } = await supabase
          .from('VisitSchedule')
          .select('*, Dorm(name, images), Inquiries(student_name)')
        ...
        const transformed = uniqueVisits.map(vs => ({
          ...
          visit_date: vs.date,
          visit_time: vs.time,
          user_name: vs.Inquiries?.student_name || 'Student',
        }));
  ...
  const SkeletonHeader = () => (
    <div className="mb-8 shimmer px-2">
      <div className="h-4 w-32 bg-gray-200 rounded mb-4" />
      <div className="h-10 w-64 bg-gray-300 rounded-2xl mb-4" />
      <div className="h-4 w-48 bg-gray-200 rounded" />
    </div>
  );

  const SkeletonCalendar = () => (
    <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm shimmer">
      ...
      <div className="grid grid-cols-7 gap-2 mb-4">
        {[1,2,3,4,5,6,7].map(i => <div key={i} className="h-3 bg-gray-100 rounded mx-auto w-8" />)}
      </div>
      <div className="grid grid-cols-7 gap-2">
        {Array.from({ length: 35 }).map((_, i) => (
          <div key={i} className="aspect-square bg-gray-50 rounded-2xl" />
        ))}
      </div>
    </div>
  );

  const SkeletonDateCard = () => (
    <div className="bg-gray-100 rounded-2xl p-6 h-40 shimmer border border-gray-100 mb-6" />
  );

  const SkeletonVisitItem = () => (
    <div className="bg-white rounded-2xl p-4 border border-gray-50 flex flex-col gap-4 shimmer mb-4">
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 pt-6 pb-32 animate-in fade-in slide-in-from-bottom-5 duration-700">
      ...
      {isLoading ? (
        ...
        <div className="mb-8">
          <h1 className="text-3xl font-black text-neutral-dark mb-1 tracking-tight">Visit Schedule</h1>
          <p className="text-gray-500 font-bold text-[10px] uppercase tracking-[0.15em] opacity-60">Manage your upcoming property tours</p>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="col-span-1 lg:col-span-2">
          {isLoading ? (
            ...
            <div className="bg-white rounded-2xl p-6 shadow-[0_15px_40px_rgba(0,0,0,0.08)] border border-gray-100">
              <div className="flex items-center justify-between mb-8">
                <h2 className="text-xl font-bold text-neutral-dark capitalize tracking-tight flex items-center gap-2">
                  <CalendarIcon className="w-5 h-5 text-primary" />
                  {format(currentMonth, 'MMMM yyyy')}
                </h2>
                <div className="flex gap-2">
                  <button onClick={prevMonth} className="p-2 rounded-full hover:bg-gray-50 transition-colors text-gray-400">
                    <ChevronLeft className="w-5 h-5" />
                  </button>
                  <button onClick={nextMonth} className="p-2 rounded-full hover:bg-gray-50 transition-colors text-gray-400">
                      ...
                      {hasVisits && (
                        <div className="absolute bottom-2 flex gap-1">
                          {dayVisits.slice(0, 3).map((_, i) => (
                            <div key={i} className={`w-1.5 h-1.5 rounded-full ${isSelected ? 'bg-white' : 'bg-secondary'}`} />
                          ))}
                        </div>
                      )}
                    </button>
        ...
        <div className="col-span-1 flex flex-col gap-6">
          {isLoading ? (
            <>
              <SkeletonDateCard />
              <div className="h-4 w-32 bg-gray-100 rounded mb-4 shimmer" />
              <SkeletonVisitItem />
              <SkeletonVisitItem />
            </>
          ) : (
            <>
              <div className="bg-gradient-to-br from-neutral-dark to-gray-900 rounded-2xl p-6 text-white shadow-xl relative overflow-hidden">
              ...
              <div className="flex-1">
                <h4 className="text-[10px] font-bold uppercase tracking-[0.2em] text-gray-400 mb-4 px-2">
                      ...
                      <div key={visit.id} className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100 group">
                        <div className="flex items-start justify-between mb-4">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-xs font-bold text-neutral-dark tracking-tight">
                              {visit.visit_time}
                            </span>
                          </div>
                          <span className="text-[8px] font-bold uppercase tracking-widest text-primary bg-primary/10 px-2 py-1 rounded-full">
                            Approved
                          </span>
                        </div>
                        ...
                        <div className="flex items-center gap-3 mb-4">
                          ...
                          <div>
                            ...
                            <Link 
                              href={`/owner/messages?id=${visit.inquiry_id}`}
                            ...
                            >
                              <MessageCircle className="w-3 h-3" /> Chat with student
                            </Link>
                          </div>
                        </div>
                        ...
                        <div className="flex gap-2">
                          <Link 
                            href={`/owner/listings/${visit.dorm_id}`}
                            className="flex-1 bg-gray-50 hover:bg-gray-100 text-neutral-dark py-2.5 rounded-2xl text-[10px] font-bold uppercase tracking-widest transition-colors flex justify-center items-center gap-1.5 border border-gray-200"
                          >
                             <Home className="w-3.5 h-3.5" /> Property
                          </Link>
                          <Link 
                            href={`/owner/messages?id=${visit.inquiry_id}`}
                            className="flex-1 bg-primary hover:bg-primary/90 text-white py-2.5 rounded-2xl text-[10px] font-bold uppercase tracking-widest shadow-lg shadow-primary/20 transition-all active:scale-95 flex justify-center items-center gap-1.5"
                          >
...
export default function OwnerCalendar() {
  return (
    <Suspense fallback={
      <div className="max-w-4xl mx-auto px-4 pt-6 pb-32 shimmer">
        <div className="h-4 w-32 bg-gray-200 rounded mb-4" />
        <div className="h-10 w-64 bg-gray-300 rounded-2xl mb-4" />
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
